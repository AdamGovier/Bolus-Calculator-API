...
export default async (req, res) => {
    ...
    if (!refreshToken) return res.status(401).json(new ErrorResponse("User is not authroised to access this resource.", {redirect: process.env.UNAUTHENTICATED_REDIRECT_PATH}));
    ...
    jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err, user) => {
        ...
        res.header('access-token', accessToken).status(200).json({message: "Token refreshed, check headers."});
    });
}